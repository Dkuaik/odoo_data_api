...
def fix_encoding(text: str) -> str:
    """
    Intenta convertir una cadena que probablemente esté en CP1252/Latin-1
    a UTF-8 de forma correcta, y limpia tabulaciones.
    """
    text = text.replace("\t", " ")
    try:
        ...
        pass
    return text
def fix_dictionary(data: dict) -> dict:
    """
    Aplica fix_encoding a todas las claves y valores
    para intentar mejorar la legibilidad (acentos, eñes).
    """
    fixed = {}
    for k, v in data.items():
        new_k = fix_encoding(k)
        new_v = fix_encoding(v)
        fixed[new_k] = new_v
    return fixed

def html_to_plain_text(html_string: str) -> str:
    """
    Recibe una cadena con contenido HTML y devuelve el texto sin etiquetas.
    """
    soup = BeautifulSoup(html_string, 'html.parser')
    # Extraer el texto puro
    plain_text = soup.get_text()
    
    # Opcional: si quieres también quitar espacios en exceso y saltos de línea
...
def texto_a_json(texto):
    ...
    for linea in lineas:
        ...
        # Verificamos si hay al menos un ":"
        if ':' in linea:
            # Separamos en dos partes usando el primer ":" que aparezca
            partes = linea.split(':', 1)
            
            # La clave o nombre del campo será lo que esté antes de ":"
            clave = partes[0].strip()
            
            # El valor es todo lo que quede después
            valor = partes[1].strip()
            
            # Guardamos en el diccionario
            resultado[clave] = valor
    
    # Convertimos el diccionario a JSON (con indentación para visualizar mejor)
